...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<TopupBannerProvider>(context, listen: false).fetchBanners();
      Provider.of<Topup_Products_Provider>(context, listen: false)
          .fetchTopupProducts();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          // আনুমানিক কনটেন্ট হাইট (প্রায়)
          final double contentHeight = 900.0;

          // 🔹 যদি কনটেন্ট ছোট হয় → footer নিচে থাকবে
          ...
          // 🔹 যদি কনটেন্ট বড় হয় → scrollable হবে এবং শেষে footer থাকবে
          else {
        ...
        Consumer<Topup_Products_Provider>(
          builder: (context, productProvider, child) {
            ...
            if (productProvider.products.isEmpty) {
              return const Padding(
                padding: EdgeInsets.all(20),
                child: Text('কোন প্রোডাক্ট পাওয়া যায়নি',
                    style: TextStyle(color: Colors.grey)),
              );
            }
